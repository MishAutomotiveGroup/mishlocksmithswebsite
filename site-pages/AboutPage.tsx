...
export default function AboutPage() {
  return (
    ...
    >
      ...
      {/* ── HERO ──────────────────────────────────────────────────────────── */}
      <section className="bg-[#121212] text-white pt-9 pb-10 px-4" data-testid="section-about-hero">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-md">
            {teamMembers.map((member) => (
              <div key={member.src} className="flex flex-col">
                ...
                <div className="bg-[#1a1a1a] border border-white/10 border-t-0 rounded-b-xl px-3 py-2.5 text-center">
                  <p className="text-white font-semibold text-sm">{member.name}</p>
                  {member.role && (
                    <p className="text-[#C79A1B] text-[10px] mt-0.5">{member.role}</p>
                  )}
      ...
      <section className="bg-white py-12 px-4" data-testid="section-about-intro">
        <div className="max-w-3xl mx-auto">
          <div className="bg-[#F7F7F4] border border-[#D8D8D3] rounded-xl p-7 sm:p-9">
            <h2 className="text-xl font-bold text-[#121212] mb-4">
              A Local Business You Can Speak to Directly
            </h2>
            <p className="text-sm text-[#121212]/70 leading-relaxed mb-3">
            ...
            {teamMembers.some(m => m.bio) && (
              <div className="mt-6 pt-6 border-t border-[#D8D8D3] flex flex-col gap-5">
                {teamMembers.filter(m => m.bio).map(m => (
                  <div key={m.name}>
                    <p className="font-semibold text-sm text-[#121212] mb-1">{m.name}</p>
                    <p className="text-sm text-[#121212]/65 leading-relaxed">{m.bio}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* ── WHAT WE DO ────────────────────────────────────────────────────── */}
      <section className="bg-[#F7F7F4] py-12 px-4" data-testid="section-about-services">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-xl font-bold text-[#121212] mb-6">What We Do</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

            {/* Vehicle Lockouts */}
            <div className="bg-white border border-[#D8D8D3] rounded-xl p-6 flex flex-col">
              <h3 className="font-bold text-[#121212] text-base mb-1">Vehicle Lockouts</h3>
              <p className="text-[#C79A1B] font-bold text-lg mb-3">£90–£110</p>
              <ul className="flex flex-col gap-2 mb-5 flex-1">
                {[
                  ...
                  <li key={item} className="flex items-start gap-2 text-sm text-[#121212]/70">
            ...
            {/* Spare Car Keys */}
            <div className="bg-white border border-[#D8D8D3] rounded-xl p-6 flex flex-col">
              <h3 className="font-bold text-[#121212] text-base mb-1">Spare Car Keys</h3>
              <p className="text-[#C79A1B] font-bold text-lg mb-3">Typically £160–£220</p>
              <ul className="flex flex-col gap-2 mb-5 flex-1">
                {[
      ...
      <section className="bg-white py-10 px-4" data-testid="section-about-who-attends">
        <div className="max-w-3xl mx-auto">
          <div className="border border-[#D8D8D3] rounded-xl p-6 sm:p-7">
            <h2 className="text-base font-bold text-[#121212] mb-3">Who Will Attend?</h2>
            <p className="text-sm text-[#121212]/65 leading-relaxed mb-5">
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                <div key={point} className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#121212]/70">{point}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-[#F7F7F4] py-8 px-4" data-testid="section-about-not-offered">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-sm font-semibold text-[#121212]/50 mb-3">Services We Don't Currently Offer</h2>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <span key={item} className="px-3 py-1.5 bg-white border border-[#D8D8D3] rounded text-sm text-[#121212]/45">
                {item}
              </span>
            ))}
          </div>
      ...
      <section className="bg-white py-12 px-4" data-testid="section-about-why">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-xl font-bold text-[#121212] mb-7">Why Customers Choose Us</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              {
                icon: <BadgeCheck size={20} className="text-[#C79A1B]" />,
                title: "Clear Pricing",
                desc: "Exact quote confirmed before we travel.",
              },
              {
                icon: <User size={20} className="text-[#C79A1B]" />,
                title: "Real Local Business",
                desc: "You're speaking directly to someone involved in carrying out the work.",
              },
              {
                icon: <CheckCircle size={20} className="text-[#C79A1B]" />,
                title: "Vehicle Compatibility Checked",
                desc: "We confirm we can help before dispatch.",
              },
              {
                icon: <Shield size={20} className="text-[#C79A1B]" />,
                title: "No Hidden Charges",
                desc: "No surprise costs after arrival.",
              },
            ].map(card => (
              <div key={card.title} className="bg-[#F7F7F4] border border-[#D8D8D3] rounded-xl p-5 flex items-start gap-4">
                <div className="shrink-0 mt-0.5">{card.icon}</div>
                <div>
                  <p className="font-semibold text-sm text-[#121212] mb-1">{card.title}</p>
                  <p className="text-sm text-[#121212]/60 leading-relaxed">{card.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── HOW WE WORK ───────────────────────────────────────────────────── */}
      <section className="bg-[#F7F7F4] py-12 px-4" data-testid="section-about-how">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-xl font-bold text-[#121212] mb-8">How We Work</h2>
      ...
      <section className="bg-white py-12 px-4" data-testid="section-about-areas">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-start gap-3 mb-4">
            <MapPin size={20} className="text-[#C79A1B] shrink-0 mt-0.5" />
            <h2 className="text-xl font-bold text-[#121212]">Serving West London</h2>
          </div>
          <p className="text-sm text-[#121212]/70 leading-relaxed mb-6 max-w-xl">
      ...
      {/* ── TRUST / PHOTOS ────────────────────────────────────────────────── */}
      <section className="bg-[#121212] py-12 px-4" data-testid="section-about-trust">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="inline-flex items-center gap-2.5 bg-white/5 border border-white/10 rounded-lg px-4 py-2.5">
            ...
            <div className="flex items-center gap-1">
              {[1,2,3,4,5].map(i => (
                <Star key={i} size={12} className="text-[#C79A1B] fill-[#C79A1B]" />
              ))}
            </div>
      ...
      <section className="bg-white py-12 px-4" data-testid="section-about-faq">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-xl font-bold text-[#121212] mb-2">About the Service</h2>
      ...
      <section className="bg-[#121212] py-14 px-4 pb-32 md:pb-14" data-testid="section-about-cta">
